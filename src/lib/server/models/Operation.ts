...
export interface OperationFilters {
    search?: string; // Пошук по імені юзера або назві карти
    typeId?: number;
    userId?: number;
    sortBy?: string;
    sortDir?: 'asc' | 'desc';
...
export class Operation {
    ...
    static async findWithFilters(filters: OperationFilters): Promise<Operation[]> {
        ...
        const allowedSorts = ['operation_id', 'quantity'];
    ...
    async save(): Promise<void> {
        if (this._id) {
            await runDBCommand(
                'UPDATE operation SET user_id=?, operation_type_id=?, videocard_id=?, quantity=? WHERE operation_id=?',
                [this._userId, this._typeId, this._videocardId, this._quantity, this._id]
            );
        } else {
            const res = await runDBCommand(
                'INSERT INTO operation (user_id, operation_type_id, videocard_id, quantity) VALUES (?, ?, ?, ?)',
                [this._userId, this._typeId, this._videocardId, this._quantity]
            ) as ResultSetHeader;
